...
int main() {
  ...
  try {
    ...
    auto held = channels.Send(kMethods, "exchangeUfvk", UfvkArgs());
    Pump(50ms);
    ...
    fake.hold_response = true;
    held = channels.Send(kMethods, "exchangeUfvk", UfvkArgs());
    Pump(50ms);
    ...
    // A short UFVK export needs no continuation while a longer one is stitched
    // from two frames.
