...
const ENVELOPE_CONTENT_HASH_DOMAIN: &[u8] = b"Vizor private state envelope content hash v1";
...
pub struct RequestAuthorization {
    ...
    /// signed envelope; GET uses SHA-256 of the empty byte string.
...
pub fn seal_object(
    ...
    let aad = envelope_aad(&reference);
    ...
    let unsigned = canonical_envelope(&reference, nonce.as_slice(), &ciphertext);
...
pub fn open_object(
    ...
    let aad = envelope_aad(&expected_reference);
    let cipher = Aes256Gcm::new_from_slice(keys.encryption_key.as_slice())
        .map_err(|_| "Failed to initialize private-state cipher".to_string())?;
    cipher
        .decrypt(
...
/// content without possessing the UFVK or decrypting the ciphertext. This
/// proves owner authorization of opaque bytes, not that the bytes decrypt to a
/// meaningful document.
...
pub fn verify_put_authorization_content(
    ...
    if authorization.method != "PUT"
        ...
        || authorization.content_hash_base64 != URL_SAFE_NO_PAD.encode(verified.content_hash)
    {
...
fn verify_encrypted_object_inner(envelope: &EncryptedObject) -> Result<VerifiedEnvelope, String> {
    ...
    let unsigned = canonical_envelope(&reference, &nonce, &ciphertext);
    ...
    let content_hash = hash_signed_envelope(&unsigned, &signature_bytes);
    Ok(VerifiedEnvelope {
        reference,
        nonce,
        ciphertext,
...
pub fn authorize_request(
    ...
    let content_hash = match (method, envelope) {
        ("PUT", Some(envelope)) => {
            let verified = verify_encrypted_object_inner(envelope)?;
            verified.content_hash.to_vec()
        }
        ("PUT", None) => {
            return Err("Private-state PUT authorization requires an envelope".to_string())
        }
        ("GET", Some(_)) => {
            return Err("Private-state GET authorization must not include an envelope".to_string())
        }
        ("GET", None) => Sha256::digest([]).to_vec(),
    ...
    if let Some(envelope) = envelope {
        if envelope.object_id != reference.object_id
            || envelope.auth_public_key_base64 != reference.auth_public_key_base64
        {
            return Err("Private-state envelope does not belong to the requested key".to_string());
        }
    }
...
fn envelope_aad(reference: &ObjectReference) -> Vec<u8> {
    let mut encoded = Vec::new();
...
fn canonical_envelope(reference: &ObjectReference, nonce: &[u8], ciphertext: &[u8]) -> Vec<u8> {
    let mut encoded = envelope_aad(reference);
...
fn hash_signed_envelope(unsigned: &[u8], signature: &[u8]) -> [u8; SHA256_LEN] {
    let mut hasher = Sha256::new();
    hasher.update(ENVELOPE_CONTENT_HASH_DOMAIN);
...
mod tests {
    ...
    #[test]
    fn envelope_round_trip_binds_object_key() {
    ...
    fn server_verifies_authorized_ciphertext_without_ufvk() {
        ...
        verify_put_authorization_content(&envelope, &authorization).unwrap();

        let mut mismatched = authorization;
        mismatched.content_hash_base64 = URL_SAFE_NO_PAD.encode([4u8; 32]);
        assert!(verify_put_authorization_content(&envelope, &mismatched).is_err());
    ...
    #[test]
    fn put_authorization_cannot_be_reused_for_another_signed_envelope() {
        ...
        verify_put_authorization_content(&first, &authorization).unwrap();
        assert!(verify_put_authorization_content(&second, &authorization).is_err());
    }
