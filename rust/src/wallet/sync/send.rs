...
fn validate_unbroadcast_migration_recovery_candidates(
    ...
    for candidate in candidates {
            ...
            != super::migration::MigrationBroadcastAttemptState::NotAttempted
        {
            ...
            // ambiguous separate-route acceptance. Expiry is the safe unlock boundary.
            if candidate.expiry_height == 0 {
                return Err(format!(
                    "Separately submitted migration transaction {} has no expiry height",
                    candidate.txid_hex
                ));
            }
            if chain_tip_height < candidate.expiry_height {
                return Err(format!(
                    "Migration recovery must wait until separately submitted transaction {} expires at block {}",
