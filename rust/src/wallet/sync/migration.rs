...
impl CustomMigrationRunConfig {
    fn validate(self) -> Result<(), String> {
        if !(1..=64).contains(&self.amount_group_count) {
            return Err("Custom migration amount group count must be between 1 and 64".to_string());
        }
        if self.parallel_schedule_count > self.amount_group_count {
            return Err(
                "Custom migration parallel schedule count cannot exceed the amount group count"
                    .to_string(),
            );
        }
        validate_custom_parallel_schedule_count(self.parallel_schedule_count)
    }
...
fn custom_migration_config_for_run_with_conn(
    ...
    let (amount_policy, amount_group_count, parallel_schedule_count, plan_seed) = conn
        .query_row(
            &format!(
                "SELECT amount_policy, custom_amount_group_count,
                        custom_parallel_schedule_count, custom_plan_seed
        ...
        "custom" => {
            let config = CustomMigrationRunConfig {
                amount_group_count: amount_group_count
                    .ok_or("Custom migration amount group count is missing")?,
                parallel_schedule_count: parallel_schedule_count
                    .ok_or("Custom migration parallel schedule count is missing")?,
                plan_seed: plan_seed
...
fn adopt_timing_policy_for_active_run(
    ...
    let custom_config = custom_migration_config_for_run_with_conn(conn, &run.run_id)?;
    let (schedule, persisted_schedule_parameters) = if let Some(config) = custom_config {
        let part_count = u32::try_from(run.target_values_zatoshi.len())
            .map_err(|_| "Custom migration part count exceeds u32".to_string())?;
        let mut rng = seeded_custom_schedule_rng(config.plan_seed);
        (
            planned_custom_transfer_schedule(
                run.target_values_zatoshi.iter().copied(),
                network,
                desired_policy,
                config.parallel_schedule_count,
                &mut rng,
            )?,
            Some(config.schedule_parameters(network, desired_policy, part_count)?),
        )
    } else {
        (
            planned_transfer_schedule_with_policy(
                run.target_values_zatoshi.iter().copied(),
                network,
                desired_policy,
                &mut OsRng,
        ...
        &format!(
            "UPDATE {RUNS_TABLE}
             SET timing_policy = ?1, schedule_json = ?2,
                 schedule_mean_delay_blocks = ?3, schedule_max_delay_blocks = ?4,
                 updated_at_ms = ?5
             WHERE run_id = ?6 AND timing_policy != 'fast_testnet'"
        ),
        params![
            desired_policy.as_str(),
            schedule_json,
            persisted_schedule_parameters.map(|parameters| parameters.0),
            persisted_schedule_parameters.map(|parameters| parameters.1),
            now,
            run.run_id,
        ],
    )
...
fn create_or_resume_migration_draft(
    ...
    let timing_policy = configured_timing_policy(network);
    let part_count = u32::try_from(target_values_zatoshi.len())
        .map_err(|_| "Custom migration part count exceeds u32".to_string())?;
    let custom_schedule_parameters = if let Some(config) = custom_config {
        ...
        Some(config.schedule_parameters(network, timing_policy, part_count)?)
    } else {
        validate_schedule_with_policy(
            approved_schedule,
            target_values_zatoshi,
    ...
    tx.execute(
        &format!(
            "INSERT INTO {RUNS_TABLE}
             (run_id, account_uuid, network, db_fingerprint, phase, created_at_ms,
              updated_at_ms, target_values_json, timing_policy, schedule_json,
              preparation_timing_policy, amount_policy, custom_amount_group_count,
              custom_parallel_schedule_count, custom_plan_seed,
        ...
        params![
            ...
            custom_config.map(|config| config.amount_group_count),
            custom_config.map(|config| config.parallel_schedule_count),
...
pub(crate) fn set_run_approved_schedule(
    ...
    if let Some(custom) = custom_migration_config_for_run_with_conn(&conn, run_id)? {
        validate_custom_schedule(
            ...
            network,
            timing_policy,
            custom.parallel_schedule_count,
        )?;
    } else {
