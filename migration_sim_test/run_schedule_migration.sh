...
RUN_LOG="$ARTIFACT_DIR/schedule-$RUN_ID.log"
OS_LOG="$ARTIFACT_DIR/schedule-$RUN_ID-rust.log"
DRIVER_LOG="$ROOT_DIR/.ironwood-regtest/migration_sim_schedule_migration_test-driver.log"
START_TIME="$(date '+%Y-%m-%d %H:%M:%S')"

mkdir -p "$ARTIFACT_DIR"

export E2E_TEST_FILE="integration_test/migration_sim_schedule_migration_test.dart"
...
if [[ -f "$DRIVER_LOG" ]]; then
  cp "$DRIVER_LOG" "$ARTIFACT_DIR/schedule-$RUN_ID-driver.log"
fi
...
if [[ -n "$db_path" && -f "$db_path" ]]; then
  sqlite3 "$db_path" ".timeout 5000" ".backup '$ARTIFACT_DIR/schedule-$RUN_ID.db'"
  "$SIM_DIR/inspect.sh" "$db_path" \
    >"$ARTIFACT_DIR/schedule-$RUN_ID-state.txt" 2>&1 || true
fi

echo "artifacts: $ARTIFACT_DIR/schedule-$RUN_ID*"
exit "$status"
