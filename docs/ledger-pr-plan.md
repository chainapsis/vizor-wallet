...
상태: C01·C02·C05·C06a는 코어 모음에 병합되었다. C03은 최신 모음 기준으로
리베이스하고 signer 리팩토링 영향을 반영해 리뷰를 준비했다.
...
준비 단계 이후 기능별 이관·검증·리뷰·모음 병합을 진행하고 있다.
현재 상태와 검증 한계는 모음 PR 본문과 리뷰 트랙 분류에서 관리한다.
