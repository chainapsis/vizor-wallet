...
const _fakeBalanceZec = String.fromEnvironment(
...
void main() {
  ...
  testWidgets(
    'submits and observes a real large-note Custom migration',
    (tester) async {
      expect(_expectedFundingZatoshiText, isNotEmpty);
      expect(_fakeBalanceZec, isEmpty);
      ...
      final approvedTargets = started.targetValuesZatoshi.toList();
      expect(started.totalCount, approvedTargets.length);
      expect(_sum(approvedTargets), lessThan(expectedFunding));
      ...
      expect(finalBalance.ironwood, _sum(approvedTargets));
      expect(finalBalance.orchard, lessThan(expectedFunding));
      expect(
        finalBalance.orchard + finalBalance.ironwood,
        lessThan(expectedFunding),
      );
      e2eLog(
...
Future<rust_sync.MigrationStatus> _driveScheduledBroadcasts(
  ...
  while (DateTime.now().isBefore(deadline)) {
    ...
    final mempool = await ironwoodDriverGet(_driverUrl, '/mempool');
    expect(mempool['size'] as int, greaterThanOrEqualTo(newlySubmitted));
    e2eLog(
...
Future<void> _waitForIdleSync(
  ...
  return pumpUntil(
    ...
    description: 'large-value wallet sync at $targetHeight',
    timeout: const Duration(minutes: 10),
  );
}
...
Future<void> _waitForIronwoodSync(
  ...
  return pumpUntil(
    ...
    description: 'active Ironwood large-value wallet sync',
    timeout: const Duration(minutes: 10),
  );
}
