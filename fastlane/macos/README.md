...
- `SPARKLE_PUBLIC_ED_KEY_MAINNET` / `SPARKLE_PUBLIC_ED_KEY_TESTNET`은 stable release에서 flavor별 앱 `Info.plist`에 주입되는 공개 Ed25519 키입니다. stable 앱은 `SUVerifyUpdateBeforeExtraction`과 `SURequireSignedFeed`도 켜서 update archive를 extraction 전에 검증하고 appcast/release notes signing을 요구하며, `SUAllowsAutomaticUpdates=NO`로 자동 다운로드/설치를 허용하지 않습니다.
...
- Sparkle delta 입력으로 쓰는 이전 release는 같은 flavor의 고정 ZIP(`Vizor-macos.zip` 또는 `Vizor-Testnet-macos.zip`)과 같은 flavor의 appcast asset을 둘 다 가진 stable release여야 합니다. fastlane은 이전 appcast XML이 그 release의 고정 ZIP enclosure를 가리키는지도 확인하며, 조건을 만족하지 않으면 delta만 생략하고 현재 full ZIP update는 계속 생성합니다.
- 현재 stable release의 appcast는 업로드 전에 현재 태그의 고정 ZIP URL과 같은 basename의 release notes markdown(`Vizor-macos.md` 또는 `Vizor-Testnet-macos.md`) URL을 가리키고, appcast와 release notes가 Sparkle EdDSA로 검증되는지 확인합니다.
