 ...
 *
 * This does not touch the wallet passcode escrow. Android 11+ can present
 * biometric-or-device-credential in one BiometricPrompt. Android 10 and lower
 * use the system device credential confirmation intent because the
 * BIOMETRIC_STRONG | DEVICE_CREDENTIAL combination is not supported there.
 */
class DeviceOwnerAuthHandler(private val activity: FragmentActivity) {
    ...
    private fun verify(reason: String, result: MethodChannel.Result) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            verifyWithBiometricPrompt(reason, result)
        } else {
            verifyWithDeviceCredentialIntent(reason, result)
        }
    }

    private fun verifyWithBiometricPrompt(reason: String, result: MethodChannel.Result) {
