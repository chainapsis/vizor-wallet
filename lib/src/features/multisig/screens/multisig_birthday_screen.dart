...
import '../../onboarding/import/import_birthday_calendar_overlay.dart';
import '../../onboarding/import/import_birthday_estimator.dart';
import '../../onboarding/import/import_birthday_unknown_height_modal.dart';
...
enum _MultisigBirthdaySubmitPhase { idle, stoppingSync, importing }
...
class MultisigBirthdayScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<MultisigBirthdayScreen> createState() =>
      _MultisigBirthdayScreenState();
}

class _MultisigBirthdayScreenState
    extends ConsumerState<MultisigBirthdayScreen> {
  ...
  MultisigBirthdayTab _activeTab = MultisigBirthdayTab.date;
  DateTime? _selectedDate;
  int? _birthdayHeight;
  bool _isEstimating = false;
  bool _isCalendarOpen = false;
  bool _isUnknownBirthdayConfirmOpen = false;
  _MultisigBirthdaySubmitPhase _submitPhase = _MultisigBirthdaySubmitPhase.idle;
  String? _metadataError;
  String? _submitError;
  DateTime? _calendarInitialDate;
  int _estimateSeq = 0;

  bool get _isSubmitting => _submitPhase != _MultisigBirthdaySubmitPhase.idle;
  ...
  Future<void> _loadMetadata() async {
    ...
    try {
      ...
      log('MultisigBirthdayScreen._loadMetadata: ERROR: $e\n$st');
      if (!mounted) return;
      setState(() {
  ...
  Future<void> _estimateSelectedDate(DateTime date) async {
    ...
    try {
      ...
      log('MultisigBirthdayScreen._estimateSelectedDate: ERROR: $e\n$st');
      if (!mounted || seq != _estimateSeq) return;
      setState(() {
        _isEstimating = false;
  ...
  void _handleTabSelected(MultisigBirthdayTab tab) {
    if (tab == MultisigBirthdayTab.blockHeight) {
      _estimateSeq++;
      setState(() {
        _activeTab = tab;
        _isEstimating = false;
    ...
    if (tab == MultisigBirthdayTab.date &&
        _selectedDate != null &&
        _birthdayHeight == null &&
        !_isEstimating) {
  ...
  Future<void> _submit({int? birthdayHeightOverride}) async {
    ...
    setState(() {
      _submitPhase = _MultisigBirthdaySubmitPhase.importing;
      _submitError = null;
    });
    ...
    try {
      ...
      if (!multisigLocalBackupCompleted(session)) {
        setState(() {
          _submitPhase = _MultisigBirthdaySubmitPhase.idle;
          _submitError =
      ...
      await runWithSyncPausedForAccountMutation(
        ...
        onStoppingSync: () {
          if (!mounted) return;
          setState(() {
            _submitPhase = _MultisigBirthdaySubmitPhase.stoppingSync;
          });
        },
        onSyncPaused: () {
          if (!mounted) return;
      ...
      log('MultisigBirthdayScreen._submit: ERROR: $e\n$st');
      if (!mounted) return;
      setState(() {
        _submitPhase = _MultisigBirthdaySubmitPhase.idle;
  ...
  int? _resolvedBirthdayHeight() {
    return switch (_activeTab) {
      MultisigBirthdayTab.date => _birthdayHeight,
      MultisigBirthdayTab.blockHeight => _validatedManualHeight,
    };
  }
  ...
  int? get _validatedManualHeight {
    ...
    if (value < minimumHeight) return null;
    final maximumHeight = _metadata?.tipHeight;
    if (maximumHeight != null && value > maximumHeight) return null;
    return value;
  }
  ...
  bool get _isSubmitEnabled {
    return switch (_activeTab) {
      MultisigBirthdayTab.date =>
        _birthdayHeight != null && !_isSubmitting && !_isEstimating,
      MultisigBirthdayTab.blockHeight =>
  ...
  Widget build(BuildContext context) {
    ...
    final buttonLabel = switch (_submitPhase) {
      _MultisigBirthdaySubmitPhase.stoppingSync => 'Stop syncing...',
      _MultisigBirthdaySubmitPhase.importing => 'Creating...',
      _MultisigBirthdaySubmitPhase.idle =>
        activeTab == MultisigBirthdayTab.date && _isEstimating
            ? 'Estimating...'
            : 'Continue',
    };
            ...
            children: [
                  ...
                  child: SizedBox(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (activeTab == MultisigBirthdayTab.date)
                          _DatePickerField(
                            width: _widgetWidth,
                        ...
                        SizedBox(
                          ...
                          child: activeTab == MultisigBirthdayTab.date
              ...
              SizedBox(
                ...
                child: Column(
                  ...
                  children: [
                    AppButton(
                      key: const ValueKey('multisig_birthday_submit_button'),
                    ...
                    AppButton(
                      key: const ValueKey('multisig_birthday_skip_button'),
...
class _BirthdayTabRow extends StatelessWidget {
  ...
  final MultisigBirthdayTab activeTab;
  final ValueChanged<MultisigBirthdayTab> onTabSelected;
    ...
    return Row(
      ...
      children: [
        _TabLabel(
          ...
          active: activeTab == MultisigBirthdayTab.date,
          onTap: () => onTabSelected(MultisigBirthdayTab.date),
        ...
        _TabLabel(
          ...
          active: activeTab == MultisigBirthdayTab.blockHeight,
          onTap: () => onTabSelected(MultisigBirthdayTab.blockHeight),
          activeColor: colors.text.accent,
          inactiveColor: colors.text.muted,
        ),
      ],
