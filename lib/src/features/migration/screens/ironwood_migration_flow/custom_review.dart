...
class _IronwoodMigrationCustomContentState
  ...
  static const _profilePresets = [4, 12, 32, 64];
  static const _concurrencyPresets = [1, 2, 4, 8, 16, 32];

  var _profileCount = 12;
  var _concurrentProfiles = 2;
  ...
  void initState() {
    ...
    _profileCount = preview?.customProfileCount ?? _profileCount;
    _concurrentProfiles =
        preview?.customConcurrentProfiles ?? _concurrentProfiles;
  ...
  IronwoodMigrationCustomPlanRequest get _request =>
      IronwoodMigrationCustomPlanRequest(
        ...
        concurrentProfiles: _concurrentProfiles,
        planSeed: _planSeed,
      );

  void _selectProfileCount(int value) {
    setState(() {
      _profileCount = value;
      _concurrentProfiles = math.min(_concurrentProfiles, value);
      _startError = null;
    });
  }

  void _selectConcurrency(int value) {
    setState(() {
      _concurrentProfiles = value;
      _startError = null;
    });
  }
  ...
  Future<void> _startCustomMigration(
    ...
    if (_isStarting || plan.isSimulated) return;
  ...
  Widget build(BuildContext context) {
    ...
    final header = Column(
      ...
      children: [
        ...
        Text(
          'Model your balance as many smaller wallets, then choose how many '
          'migration schedules run in parallel. The expected note '
          'distribution and timeline update with every change.',
          textAlign: TextAlign.center,
    ...
    final controls = Column(
      ...
      children: [
        _CustomPresetControl(
          key: const ValueKey('custom_migration_profile_control'),
          optionKeyPrefix: 'custom_migration_profile',
          title: 'Smaller wallet profiles',
          valueLabel: '$_profileCount profiles',
          value: _profileCount,
          body:
              'Model your balance as if it belonged to this many smaller '
              'wallets. More profiles, smaller notes.',
          options: _profilePresets,
          onSelected: _selectProfileCount,
        ...
        _CustomPresetControl(
          key: const ValueKey('custom_migration_concurrency_control'),
          optionKeyPrefix: 'custom_migration_concurrency',
          title: 'Parallel migration schedules',
          valueLabel: '$_concurrentProfiles at once',
          value: _concurrentProfiles,
          body:
              'Model this many users migrating at the same time. More '
              'parallel schedules shorten the overall timeline.',
          options: _concurrencyPresets
              .where((value) => value <= _profileCount)
              .toList(),
          onSelected: _selectConcurrency,
        ),
      ],
    );
            ...
            children: [
              ...
              Center(
                child: AppButton(
                  ...
                  onPressed: plan.isSimulated || _isStarting
                      ? null
