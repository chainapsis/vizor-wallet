...
class AppSecureStore {
  ...
  final _walletMutationLock = _AsyncLock();
  var _walletSessionEpoch = 0;
  var _walletResetInProgress = false;
  String? _sessionPassword;

  bool get hasSessionPassword => _sessionPassword != null;

  /// Captures the identity of the wallet session that owns a provider/store.
  ///
  /// Callers must retain this value instead of looking it up again when an
  /// asynchronous operation eventually persists. A reset advances the epoch,
  /// so work owned by the disposed ProviderScope cannot write into the next
  /// wallet session.
  int captureWalletSessionEpoch() => _walletSessionEpoch;

  bool isWalletSessionCurrent(int epoch) =>
      !_walletResetInProgress && epoch == _walletSessionEpoch;

  Future<String?> readWalletString(String key, {required int epoch}) {
    return _walletMutationLock.run(() async {
      if (!isWalletSessionCurrent(epoch)) return null;
      return readString(key);
    });
  }

  Future<String?> readWalletPlain(String key, {required int epoch}) {
    return _walletMutationLock.run(() async {
      if (!isWalletSessionCurrent(epoch)) return null;
      return readPlain(key);
    });
  }

  Future<void> writeWalletString(
    String key,
    String value, {
    required int epoch,
  }) {
    return _walletMutationLock.run(() async {
      if (!isWalletSessionCurrent(epoch)) return;
      await writeString(key, value);
    });
  }

  Future<void> writeWalletPlain(
    String key,
    String value, {
    required int epoch,
  }) {
    return _walletMutationLock.run(() async {
      if (!isWalletSessionCurrent(epoch)) return;
      await writePlain(key, value);
    });
  }

  Future<void> deleteWalletKey(String key, {required int epoch}) {
    return _walletMutationLock.run(() async {
      if (!isWalletSessionCurrent(epoch)) return;
      await delete(key);
    });
  ...
  Future<void> deleteWalletPlainKeysWithPrefix(
    String prefix, {
    required int epoch,
  }) {
    return _walletMutationLock.run(() async {
      if (!isWalletSessionCurrent(epoch)) return;
      await deletePlainKeysWithPrefix(prefix);
    });
  }

  Future<void> writeWalletAccountMnemonic(
    String accountUuid,
    String mnemonic, {
    required int epoch,
  ...
  Future<void> deleteWalletAccountMnemonic(
    String accountUuid, {
    required int epoch,
  }) {
    return _walletMutationLock.run(() async {
      if (!isWalletSessionCurrent(epoch)) return;
      await deleteAccountMnemonic(accountUuid);
    });
  }

  Future<void> deleteWalletVotingHotkeysForAccount(
    String accountUuid, {
    required int epoch,
  }) {
    return _walletMutationLock.run(() async {
      if (!isWalletSessionCurrent(epoch)) return;
      await deleteVotingHotkeysForAccount(accountUuid);
    });
  }

  /// Atomically closes the current wallet storage session and wipes it.
  ///
  /// Only explicitly listed install preferences survive. Unknown and future
  /// keys are deleted by default so a newly introduced wallet secret cannot be
  /// retained accidentally. The reset flag is set before this method first
  /// awaits, preventing new current-session writes from joining behind it.
  Future<void> resetWalletStorage({
    required int epoch,
    Set<String> preservedInstallPreferenceKeys = const {},
  }) async {
    if (epoch != _walletSessionEpoch || _walletResetInProgress) {
      throw StateError('Wallet storage session is no longer current.');
    }
    _walletResetInProgress = true;
    var committed = false;
    try {
      await _walletMutationLock.run(() async {
        if (epoch != _walletSessionEpoch) {
          throw StateError('Wallet storage session changed during reset.');
        }
        final preserved = <String, String>{};
        for (final key in preservedInstallPreferenceKeys) {
          final value = await readPlain(key);
          if (value != null) preserved[key] = value;
        }
        await deleteAll();
        // The destructive boundary has committed. Advance before restoring
        // preferences so a restore failure can never make old providers valid
        // against an already-wiped store.
        _walletSessionEpoch += 1;
        committed = true;
        for (final entry in preserved.entries) {
          await writePlain(entry.key, entry.value);
        }
      });
    } finally {
      // A pre-wipe failure leaves the old durable wallet retryable, so its
      // epoch is kept. Once deleteAll commits, old owners remain stale even if
      // restoring an install preference fails.
      _walletResetInProgress = false;
      if (!committed) {
        assert(_walletSessionEpoch == epoch);
      }
    }
  }
