...
import 'voting_snapshot_preparation_provider.dart';
...
class VotingSessionNotifier extends AsyncNotifier<VotingSessionState> {
  ...
  Future<bool> prepareDelegationSnapshot({required String accountUuid}) async {
    if (!ref.mounted || _isDisposed) return false;
    final key = _snapshotPreparationKey(accountUuid);
    final coordinator = ref.read(votingSnapshotPreparationCoordinatorProvider);
    final existing = coordinator.inFlight(key);
    if (existing != null) return existing;

    return _runSnapshotPreparationForAccount(
      accountUuid,
  ...
  Future<bool> _runSnapshotPreparationForAccount(
    String accountUuid, {
    required VotingSnapshotPreparationCoordinator coordinator,
    required VotingSessionKey key,
  }) async {
    final generation = _sessionGeneration;
    late final _VotingSessionContext context;
    try {
      context = await _loadContext(_roundId, expectedGeneration: generation);
    } on _StaleVotingSessionAction {
      return false;
    }
    if (!_isCurrentSnapshotPreparationContext(context, accountUuid) ||
    ...
    if (current == null || !current.hasConfirmedVotingEligibility) {
      debugPrint(
        '[zcash] Voting: delegation snapshot preparation skipped '
    ...
    // Capture every dependency before handing work to the app-scoped
    // coordinator. The operation may outlive this notifier and must not touch
    // its Ref after navigation or provider disposal.
    final rust = ref.read(votingRustApiProvider);
    final roundContext = _apiRoundContext(context);
    final pirServerUrl = _transportUrl(pirEndpoint);
    return coordinator.run(
      key,
      () => _runDelegationSnapshotPreparation(
        rust: rust,
  ...
  Future<void> delegatePendingBundles({String? mnemonic}) {
    return _enqueue(() async {
      ...
      _logDelegationStart('software action entered');
      var current = await future;
      var context = await _loadContext(_roundId);
      _logDelegationStart(
        'round=${context.round.roundId} context loaded '
        'elapsed=${formatElapsedSeconds(actionTimer.elapsed)} '
        'phase=${current.phase.name} '
        'pirEndpoint=${current.pirEndpoint != null} '
        'eligibleWeight=${current.eligibleWeightZatoshi != null} '
        'pendingBundles=${context.resumePlan.pendingDelegationBundleIndexes.length}',
      );
      ...
      current = await _resolvePirForPreparedBundles(
        current: current,
        context: context,
        plan: plan,
      );
      final needsPrepare =
          _needsFreshDelegationWork(plan, roundPlan) &&
          _needsDelegationPreparation(current, plan);
      if (needsPrepare) {
        _logDelegationStart(
          'round=${context.round.roundId} prepare start '
          'elapsed=${formatElapsedSeconds(actionTimer.elapsed)}',
        );
        await _prepareDelegationUnlocked();
        _logDelegationStart(
          'round=${context.round.roundId} prepare done '
          'elapsed=${formatElapsedSeconds(actionTimer.elapsed)}',
        );
      ...
      var delegationBundleIndexes = plan.pendingDelegationBundleIndexes
          .toList();
      if (roundPlanNeedsDraftSetup(roundPlan)) {
        final snapshotStatus = await ref
            .read(votingRustApiProvider)
            .getDelegationSnapshotStatus(ctx: _apiRoundContext(context));
        if (!snapshotStatus.complete) {
          throw StateError('Delegation snapshot preparation is incomplete.');
        }
        delegationBundleIndexes = List<int>.generate(
          snapshotStatus.bundleCount,
          (index) => index,
          growable: false,
        );
      }
      ...
      if (hasPendingBundles) {
        ...
        current = nextState;
        _logDelegationStart(
          'round=${context.round.roundId} phase=delegating '
          'pendingBundles=${plan.pendingDelegationBundleIndexes.length} '
          'elapsed=${formatElapsedSeconds(actionTimer.elapsed)}',
        );
      }
      if (hasPendingBundles) {
        await _awaitSnapshotPreparationIfRunning(context);
        _throwIfContextStale(context, 'delegation-snapshot');
      }
      final hotkeyTimer = Stopwatch()..start();
      ...
      if (hasPendingBundles) {
        _logDelegationStart(
          'round=${context.round.roundId} hotkey ready '
          'elapsed=${formatElapsedSeconds(actionTimer.elapsed)} '
          'hotkey=${formatElapsedSeconds(hotkeyTimer.elapsed)}',
        );
      }
      ...
      _logDelegationStart(
        'round=${context.round.roundId} confirm-submitted done '
        'elapsed=${formatElapsedSeconds(actionTimer.elapsed)} '
        'confirm=${formatElapsedSeconds(confirmTimer.elapsed)} '
        'alreadyConfirmed=${completedBundleIndexes?.length ?? 0}',
      );
      ...
      try {
        completedBundleIndexes.addAll(
          await _runDelegationBundleBatch(
            ...
            progress: progress,
            logLabel: 'software',
            prove: (publishProgress) async {
              _throwIfContextStale(context, 'delegation-proof');
              _logDelegationStart(
                'round=${context.round.roundId} '
                'bundles=${plan.pendingDelegationBundleIndexes} '
                'rust-round-stream subscribe '
                'elapsed=${formatElapsedSeconds(actionTimer.elapsed)}',
              );
              List<rust_wire.SignedDelegationPayloadView>? signedPayloads;
              await for (final event
                  in rust.buildProveAndSignDelegationRoundWithProgress(
                ...
                signedPayloads =
                    event.signedDelegationPayloads ?? signedPayloads;
                final bundleIndex = event.bundleIndex;
              ...
              return signedPayloads ??
                  (throw StateError(
                    'Delegation proof completed without submission payloads.',
                  ));
            },
          ),
        );
  ...
  Future<void> skipRemainingKeystoneBundles() {
    return _enqueue(() async {
      ...
      final retainedEligibleWeight = await ref
          .read(votingRustApiProvider)
      ...
      _setStateForContext(
        ...
        (state.value ?? current).copyWith(
          ...
          resumePlan: refreshedPlan,
          roundPlan: refreshedRoundPlan,
          eligibleWeightZatoshi: retainedEligibleWeight,
          ...
          await _runDelegationBundleBatch(
            ...
            prove: (publishProgress) async {
              _logDelegationStart(
                'round=${context.round.roundId} '
                'bundles=${plan.pendingDelegationBundleIndexes} '
                'keystone rust-round-stream subscribe',
              );
              List<rust_wire.SignedDelegationPayloadView>? signedPayloads;
              await for (final event
                  in rust
                      .buildProveDelegationRoundWithKeystoneSignaturesWithProgress(
                        ...
                        storedHotkeySecret: storedHotkeySecret!,
                        signatures: [
                          for (final bundleIndex
                              in plan.pendingDelegationBundleIndexes)
                            rust_api.ApiDelegationSignatureInput(
                              bundleIndex: bundleIndex,
                              sig: Uint8List.fromList(
                                signatures[bundleIndex]!.sig,
                              ),
                              sighash: Uint8List.fromList(
                                signatures[bundleIndex]!.sighash,
                              ),
                            ),
                        ],
                      )) {
              ...
              for (final submission in submissions) {
                final bundleIndex = submission.bundleIndex;
                final signature = signatures[bundleIndex];
                if (signature == null) {
                  throw StateError(
                    'Keystone delegation result contains unexpected bundle '
                    '$bundleIndex.',
                  );
                }
  ...
  void _logDelegationStart(String message) {
    debugPrint('[zcash] Voting: delegation-start $message');
  }
  ...
  Future<Set<int>> _runDelegationBundleBatch({
    ...
    required Map<int, VotingSessionProgress> progress,
    required Future<List<rust_wire.SignedDelegationPayloadView>> Function(
      void Function(VotingSessionProgress progress) publishProgress,
    )
    prove,
    required String logLabel,
  }) async {
    if (bundleIndexes.isEmpty) return <int>{};
    final batchTimer = Stopwatch()..start();
    final proofWallTimer = Stopwatch()..start();
    final timers = {
      for (final bundleIndex in bundleIndexes)
        bundleIndex: Stopwatch()..start(),
    };
    var loggedFirstProgress = false;
    var loggedWheelMoving = false;

    void publishProgress(VotingSessionProgress update) {
      final bundleIndex = update.bundleIndex;
      if (bundleIndex == null) return;
      final previous = progress[bundleIndex];
      progress[bundleIndex] = update;
      final sinceBatch = formatElapsedSeconds(batchTimer.elapsed);
      final sinceBundle = formatElapsedSeconds(
        timers[bundleIndex]?.elapsed ?? Duration.zero,
      );
      if (!loggedFirstProgress) {
        loggedFirstProgress = true;
        debugPrint(
          '[zcash] Voting: $logLabel delegation first-progress-event '
          'round=${context.round.roundId} bundle=$bundleIndex '
          'phase=${update.phase} proofProgress=${update.proofProgress} '
          'sinceBatch=$sinceBatch sinceBundle=$sinceBundle',
        );
      } else if (previous?.phase != update.phase) {
        debugPrint(
          '[zcash] Voting: $logLabel delegation progress-phase '
          'round=${context.round.roundId} bundle=$bundleIndex '
          'phase=${update.phase} proofProgress=${update.proofProgress} '
          'sinceBatch=$sinceBatch sinceBundle=$sinceBundle',
        );
      }
      final proofProgress = update.proofProgress;
      if (!loggedWheelMoving && proofProgress != null && proofProgress > 0) {
        loggedWheelMoving = true;
        debugPrint(
          '[zcash] Voting: $logLabel delegation wheel-moving '
          'round=${context.round.roundId} bundle=$bundleIndex '
          'phase=${update.phase} proofProgress=$proofProgress '
          'sinceBatch=$sinceBatch sinceBundle=$sinceBundle',
        );
      }
      _setStateForContext(
    ...
    final submissions = await prove(publishProgress);
    final submissionsByBundle = {
      for (final submission in submissions) submission.bundleIndex: submission,
    };
    if (submissions.length != bundleIndexes.length ||
        submissionsByBundle.length != bundleIndexes.length ||
        !bundleIndexes.every(submissionsByBundle.containsKey)) {
      throw StateError(
        '$logLabel delegation results do not match the requested bundles.',
      );
    }
    debugPrint(
      '[zcash] Voting: $logLabel delegation round proof completed '
    ...
    for (final bundleIndex in bundleIndexes) {
      try {
        ...
        final txHash = await _submitDelegation(
          context: context,
          bundleIndex: bundleIndex,
          submission: submissionsByBundle[bundleIndex]!,
        );
  ...
  Future<bool> _runDelegationSnapshotPreparation({
    required VotingRustApi rust,
    required rust_api.ApiVotingRoundContext roundContext,
    required String roundId,
    required String pirServerUrl,
  }) async {
    final timer = Stopwatch()..start();
    debugPrint(
      '[zcash] Voting: delegation snapshot preparation start '
      'round=$roundId',
    );
    try {
      rust.warmVotingProvingCaches();
      final before = await rust.getDelegationSnapshotStatus(ctx: roundContext);
      if (before.complete) {
        debugPrint(
          '[zcash] Voting: delegation snapshot already complete '
          'round=$roundId bundles=${before.bundleCount}',
        );
        return true;
      }
      final result = await rust.prepareDelegationSnapshot(
        ctx: roundContext,
        pirServerUrl: pirServerUrl,
      );
      final status = await rust.getDelegationSnapshotStatus(ctx: roundContext);
      debugPrint(
        '[zcash] Voting: delegation snapshot preparation completed '
        'round=$roundId bundles=${status.bundleCount} '
        'complete=${status.complete} alreadyComplete=${result.alreadyComplete} '
        'witnessesCached=${result.witnessesCached} '
        'witnessesGenerated=${result.witnessesGenerated} '
        'pirCached=${result.pirCached} pirFetched=${result.pirFetched} '
        'elapsed=${formatElapsedSeconds(timer.elapsed)}',
      );
      return status.complete;
    } catch (e) {
      debugPrint(
        '[zcash] Voting: delegation snapshot preparation failed '
        'round=$roundId '
  ...
  Future<void> _awaitSnapshotPreparationIfRunning(
    _VotingSessionContext context,
  ) async {
    if (!ref.mounted || !_isCurrentContext(context)) return;
    final preparation = ref
        .read(votingSnapshotPreparationCoordinatorProvider)
        .inFlight(_snapshotPreparationKey(context.accountUuid));
    if (preparation == null) {
      debugPrint(
        '[zcash] Voting: delegation snapshot preparation not in-flight '
        'round=${context.round.roundId}',
      );
      return;
    }
    ...
    debugPrint(
      '[zcash] Voting: waiting for in-flight delegation snapshot preparation '
      'round=${context.round.roundId}',
    );
    await preparation;
    debugPrint(
      '[zcash] Voting: in-flight delegation snapshot preparation joined '
      'round=${context.round.roundId} '
      'elapsed=${formatElapsedSeconds(waitTimer.elapsed)}',
    );
  }
  ...
  VotingSessionKey _snapshotPreparationKey(String accountUuid) {
    return VotingSessionKey(roundId: _roundId, accountUuid: accountUuid);
  }
  ...
  Future<void> _enqueue(
    ...
    final next = _operation.then((_) async {
      final queueWait = queuedAt.elapsed;
      if (queueWait.inMilliseconds >= 50) {
        debugPrint(
          '[zcash] Voting: session-queue wait '
          'elapsed=${formatElapsedSeconds(queueWait)}',
        );
      }
  ...
  static bool _needsDelegationPreparation(
    VotingSessionState state,
    VotingResumePlan plan,
  ) {
    return state.pirEndpoint == null ||
        (state.eligibleWeightZatoshi == null &&
            plan.pendingDelegationBundleIndexes.isEmpty);
  }

  Future<VotingSessionState> _resolvePirForPreparedBundles({
    ...
    try {
      ...
      _setError(
        _pirSnapshotMismatchMessage(error),
        cause: error,
        pirDiagnostics: error.diagnostics,
        context: context,
      );
      return current;
    } catch (error) {
      _setError(
        'Failed to resolve PIR endpoint.',
        cause: error,
        context: context,
      );
      return current;
    }
    _throwIfContextStale(context, 'prepared-bundle-pir-resolution');
    ...
    _setStateForContext(context, resolved);
    return resolved;
  }
  ...
  Future<void> _prepareKeystoneSigningUnlocked() async {
    ...
    if (_needsDelegationPreparation(current, context.resumePlan)) {
      await _prepareDelegationUnlocked();
      current = await future;
    ...
    if (roundPlanNeedsDraftSetup(roundPlan)) {
      ...
      unsignedBundleIndexes = List<int>.generate(
        snapshotStatus.bundleCount,
        (index) => index,
        growable: false,
    ...
    try {
      requests = await rust.finishDelegationRoundPreparation(
        ctx: _apiRoundContext(context),
        storedHotkeySecret: storedHotkeySecret,
        bundleIndexes: unsignedBundleIndexes,
      );
    } catch (error) {
  ...
  Future<void> _prepareDelegationUnlocked() async {
    ...
    final context = await _loadContext(_roundId);
    _logDelegationStart(
      'round=${context.round.roundId} prepare-unlocked start '
      'phase=${current.phase.name}',
    );
    ref.read(votingRustApiProvider).warmVotingProvingCaches();
    final walletTimer = Stopwatch()..start();
    await _waitUntilWalletReadyForVoting(context);
    _logDelegationStart(
      'round=${context.round.roundId} prepare wallet-ready '
      'elapsed=${formatElapsedSeconds(prepareTimer.elapsed)} '
      'walletWait=${formatElapsedSeconds(walletTimer.elapsed)}',
    );
    _setStateForContext(
      context,
      current.copyWith(
    ...
    late final PirSnapshotResolution resolution;
    final pirTimer = Stopwatch()..start();
    try {
    ...
    _logDelegationStart(
      'round=${context.round.roundId} prepare pir-resolved '
      'endpoint=${resolution.endpoint} '
      'elapsed=${formatElapsedSeconds(prepareTimer.elapsed)} '
      'pir=${formatElapsedSeconds(pirTimer.elapsed)}',
    );
    ...
    await _awaitSnapshotPreparationIfRunning(context);
    _throwIfContextStale(context, 'delegation-snapshot-preparation');
    final rust = ref.read(votingRustApiProvider);
    final roundContext = _apiRoundContext(context);
    var snapshotStatus = await rust.getDelegationSnapshotStatus(
      ctx: roundContext,
    );
    if (!snapshotStatus.complete) {
      final prepared = await ref
          .read(votingSnapshotPreparationCoordinatorProvider)
          .run(
            _snapshotPreparationKey(context.accountUuid),
            () => _runDelegationSnapshotPreparation(
              rust: rust,
              roundContext: roundContext,
              roundId: context.round.roundId,
              pirServerUrl: _transportUrl(resolution.endpoint),
            ),
          );
      _throwIfContextStale(context, 'delegation-snapshot-preparation');
      snapshotStatus = await rust.getDelegationSnapshotStatus(
        ctx: roundContext,
      );
      if (!prepared || !snapshotStatus.complete) {
        throw StateError('Delegation snapshot preparation did not complete.');
      }
    }
    _logDelegationStart(
      'round=${context.round.roundId} prepare snapshot-ready '
      'bundles=${snapshotStatus.bundleCount} '
      'elapsed=${formatElapsedSeconds(prepareTimer.elapsed)} '
  ...
  Future<String> _accountUuidForSession({bool checkStaleAction = true}) async {
  ...
  Future<bool> _isHardwareAccountForSession({
    bool checkStaleAction = true,
  }) async {
  ...
  Future<void> _waitUntilWalletReadyForVoting(
    ...
    while (true) {
      ...
      if (readiness.isReady) {
        ...
        debugPrint(
          '[zcash] Voting: wallet-sync-ready '
          'round=${context.round.roundId} '
  ...
  bool _isCurrentSnapshotPreparationContext(
    _VotingSessionContext context,
