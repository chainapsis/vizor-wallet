...
DRIVER_LOG="$ROOT_DIR/.regtest/shield-transparent-driver.log"
...
raise SystemExit("Timed out waiting for shield transparent E2E driver")
PY

echo "running Flutter macOS shield-transparent integration test"
set +e
fvm flutter test \
  integration_test/regtest_shield_transparent_test.dart \
  -d "$FLUTTER_DEVICE" \
...
if [[ "$status" -ne 0 ]]; then
  echo "shield transparent driver log:" >&2
