...
run_test "1/13 create wallet via passcode onboarding and sync" \
  "scripts/e2e/flutter-ios-regtest-mobile-create-sync.sh"

run_test "2/13 import funded wallet and sync balance" \
  "scripts/e2e/flutter-ios-regtest-mobile-import-sync.sh"

run_test "3/13 manage accounts and rotate the passcode" \
  "scripts/e2e/flutter-ios-regtest-mobile-account-management.sh"

run_test "4/13 import two accounts and send shielded funds" \
  "scripts/e2e/flutter-ios-regtest-mobile-multi-account-send.sh"

run_test "5/13 show mempool receives in activity" \
  "scripts/e2e/flutter-ios-regtest-mobile-mempool-receive.sh"

run_test "6/13 fall back from unavailable endpoint" \
  "scripts/e2e/flutter-ios-regtest-mobile-fallback-endpoint.sh"

run_test "7/13 fall back from slow-height primary and recover" \
  "scripts/e2e/flutter-ios-regtest-mobile-slow-height-fallback.sh"

run_test "8/13 migrate the minimum viable Orchard balance to Ironwood" \
  "scripts/e2e/flutter-ios-regtest-mobile-ironwood-migration.sh"

run_test "9/13 migrate twenty Orchard notes through two split stages" \
  "scripts/e2e/flutter-ios-regtest-mobile-ironwood-migration-many-notes.sh"

run_test "10/13 isolate an Ironwood migration across two accounts" \
  "scripts/e2e/flutter-ios-regtest-mobile-ironwood-migration-multi-account.sh"

run_test "11/13 rebuild an Ironwood migration after a chain reorg" \
  "scripts/e2e/flutter-ios-regtest-mobile-ironwood-migration-reorg.sh"

run_test "12/13 resume an Ironwood migration after process restart" \
