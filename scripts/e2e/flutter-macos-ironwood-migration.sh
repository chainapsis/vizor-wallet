...
if [[ -n "${ZCASH_E2E_ACCOUNT_IMPORT_MIGRATION_PHASE:-}" ]]; then
  scenario_define_args+=(
    --dart-define=ZCASH_E2E_ACCOUNT_IMPORT_MIGRATION_PHASE="$ZCASH_E2E_ACCOUNT_IMPORT_MIGRATION_PHASE"
  )
fi
...
flutter_args+=(
  ...
  --dart-define=ZCASH_E2E_FIRST_UNLOCK_MNEMONIC_KEYCHAIN="${ZCASH_E2E_FIRST_UNLOCK_MNEMONIC_KEYCHAIN:-false}"
