...
CHECKPOINT_NAME="${E2E_LARGE_VALUE_CHECKPOINT:-custom-migration-10000000-zec}"
...
if [[ ! -f "$CHECKPOINT_PATH" ]]; then
  "$ROOT_DIR/scripts/ironwood-regtest/prepare-large-value-checkpoint.sh" \
    "${E2E_LARGE_VALUE_FUNDING_AMOUNT:-10000000.0002}" \
    "$CHECKPOINT_NAME"
else
  "$ROOT_DIR/scripts/ironwood-regtest/build-large-value-zcashd.sh"
fi
if [[ -z "${E2E_ORCHARD_FUNDING_ZATOSHI:-}" ]]; then
  E2E_ORCHARD_FUNDING_ZATOSHI="$(
    tar -xOzf "$CHECKPOINT_PATH" \
      "$(basename "$IRONWOOD_STATE_DIR")/large-value-manifest.json" |
      python3 -c '
import json
import sys

value = json.load(sys.stdin).get("fundingZatoshi")
if not isinstance(value, int) or value <= 0:
    raise SystemExit("checkpoint manifest has no valid fundingZatoshi")
print(value)
'
  )"
fi
...
export E2E_ORCHARD_FUNDING_ZATOSHI
