...
void main() {
  test('classifies only rebuildable Ledger request failures', () {
    expect(
      ledgerRequestExceedsCapacity(
        StateError('Ledger supports at most 32 transparent inputs; found 33'),
      ),
      isTrue,
    );
    expect(
    ...
    expect(ledgerRequestNeedsRebuilding(StateError('status 0x6986')), isTrue);
    expect(
      ledgerRequestNeedsRebuilding(StateError('network unavailable')),
      isFalse,
    );
  });
